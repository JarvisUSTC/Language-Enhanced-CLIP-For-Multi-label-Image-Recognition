...
class CustomDistributedSamplerWrapper(DistributedSampler):  
    def __init__(self, dataset, sampler, num_replicas=None, rank=None, seed=0):  
        super().__init__(dataset, num_replicas, rank, seed)  
        self.sampler = sampler

    def __iter__(self):  
        num_samples_per_rank = len(self.sampler) // self.num_replicas  
        start = self.rank * num_samples_per_rank  
        end = start + num_samples_per_rank  
        return iter(list(self.sampler)[start:end])
