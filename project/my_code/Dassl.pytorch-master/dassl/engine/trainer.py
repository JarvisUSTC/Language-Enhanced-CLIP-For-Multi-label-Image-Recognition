...
class SimpleTrainer(TrainerBase):
    ...
    def train(self):
        ...
        for self.epoch in range(self.start_epoch, self.max_epoch):
            ...
            self.run_epoch()
            self.after_epoch()
            if self.epoch + 1 == self.cfg.TRAIN.early_stop_epoch:
                print(f"Early Stopped at epoch {self.epoch + 1}")
                break
        self.after_train()
    ...
    def parse_batch_test(self, batch):
        ...
        if 'img_blocks' in batch.keys():
            input_block = batch['img_blocks']
        ...
        return input, label, input_block
