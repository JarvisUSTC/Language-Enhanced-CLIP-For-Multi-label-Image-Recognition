...
def ranking_loss_with_cooccurrence(y_pred, y_true, cooccurrence, scale_ = 2.0, margin_ = 1):
    y_pred *= scale_  
    y_true_ = y_true.float()  
    tmp = margin_ - y_pred[:, None, :] + y_pred[:, :, None]  
 
    cooccurrence_weights = (1 / (cooccurrence+1e-6)).log()
    identity_matrix = torch.eye(cooccurrence_weights.shape[0], cooccurrence_weights.shape[1]).to(cooccurrence_weights.device)
    cooccurrence_weights = cooccurrence_weights * (1 - identity_matrix)
    cooccurrence_weights = cooccurrence_weights / cooccurrence_weights.mean(-1)[:,None]

    partial_losses = torch.maximum(torch.zeros_like(tmp), tmp) * cooccurrence_weights  
...
def ranking_loss_reweighting(y_pred, y_true, class_weights, scale_ = 2.0, margin_ = 1):  
    y_pred *= scale_  
    y_true_ = y_true.float()  
    tmp = margin_ - y_pred[:, None, :] + y_pred[:, :, None]  
    partial_losses = torch.maximum(torch.zeros_like(tmp), tmp)  
    ...
    # 根据类别权重调整损失  
    weighted_losses = partial_losses * y_true_[:, None, :] * (1 - y_true_[:, :, None]) * class_weights[None, None, :]  
    loss = torch.sum(weighted_losses, dim=-1)  
    loss = torch.sum(loss, dim=-1)  
