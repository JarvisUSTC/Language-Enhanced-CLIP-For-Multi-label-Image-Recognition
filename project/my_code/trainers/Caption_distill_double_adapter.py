...
class TextEncoder(nn.Module):
    def __init__(self, clip_model):
        ...
        ########################################
        # self.text_adapter = Adapter(512, 4).to(clip_model.dtype)
...
class AdapterTextEncoder(nn.Module):
    def __init__(self, clip_model):
        super().__init__()
        self.transformer = clip_model.transformer
        self.positional_embedding = clip_model.positional_embedding
...
class AdapterDenseCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model, return_interm_layers=False):
        super().__init__()
        self.prompt_learner = PromptLearner(cfg, classnames, clip_model)
        self.tokenized_prompts = self.prompt_learner.tokenized_prompts
        self.text_encoder = TextEncoder(clip_model)
        self.adapter_text_encoder = AdapterTextEncoder(clip_model)
        # ########################################
        # ## add bottleneck Adapter
        # ########################################
        # self.text_adapter = Adapter(1024, 4).to(clip_model.dtype)
    ...
    def forward(self, image=None, captions=None, if_test=False):
        if if_test:        
            ...
            text_features = self.adapter_text_encoder(prompts, tokenized_prompts)
            text_features_neg = self.adapter_text_encoder(prompts_double, tokenized_prompts)
            
            # ######################################################
            # ## Adapter
            # #####################################################
            # ratio = 0.2
            # text_features_adapter = self.text_adapter(text_features)
            # text_features_neg_adapter = self.text_adapter(text_features_neg)
            
            # text_features = ratio * text_features_adapter + (1-ratio) * text_features
            # text_features_neg = ratio * text_features_neg_adapter + (1-ratio) * text_features_neg
...
class Caption_distill_double_adapter(TrainerX):
    ...
    def test(self, split=None):
        ...
        print("    ".join([f"{k}: {v:.2f}" for k, v in results.items()]))
    ...
    def build_model(self):
        ...
        print("Building custom AdapterCLIP")
        # self.model = CustomCLIP(cfg, classnames, clip_model)
        self.model = AdapterDenseCLIP(cfg, classnames, clip_model)
