...
def get_coco_test(root, object_categories):
    coco_instance_json_file = os.path.join(root, "comp_instances_val2014.json")
    coco = COCO(coco_instance_json_file)
    valset_ids = coco.getImgIds()
    ...
    test_imgdir = [os.path.join(root, 'dataset_A/{}'.format(coco.loadImgs(ids = imgid)[0]['file_name'])) for imgid in valset_ids]
    test_label = torch.zeros((len(valset_ids), cls_num), dtype=torch.long)
    for idx, imgid in enumerate(valset_ids):
